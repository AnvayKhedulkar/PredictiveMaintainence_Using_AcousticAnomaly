...
logger.info("Loading Autoencoder checkpoint...")
AE_CKPT = torch.load(MODELS_DIR / "autoencoder.pt", map_location="cpu")
logger.info("Autoencoder checkpoint loaded.")

logger.info("Loading AE scaler...")
    ...
    <head>
        ...
        <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .file-name { font-size: 0.85rem; color: #6366f1; margin-top: 8px; min-height: 20px; }
            button[type="submit"] {
                width: 100%;
                padding: 14px;
                background: #6366f1;
            ...
            .loading { text-align: center; color: #6366f1; font-size: 0.9rem; margin-top: 20px; display: none; }
        </style>
    </head>
    <body>
        <div class="card">
            ...
            <div id="result">
                ...
                <div class="scores">
                    <table>
                        <tr><td>Isolation Forest Score</td><td id="ifScore"></td></tr>
                        <tr><td>IF Threshold</td><td id="ifThreshold"></td></tr>
                        <tr><td>IF Decision</td><td id="ifDecision"></td></tr>
                        <tr><td colspan="2" style="padding:6px 0;"></td></tr>
                        <tr><td>Autoencoder Score</td><td id="aeScore"></td></tr>
                        <tr><td>AE Threshold</td><td id="aeThreshold"></td></tr>
                        <tr><td>AE Decision</td><td id="aeDecision"></td></tr>
                    </table>
                </div>
            </div>
        </div>
        <script>
            ...
            document.getElementById('uploadForm').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    labelEl.className   = 'result-label ' + (label === 'normal' ? 'label-normal' : 'label-anomaly');
                    document.getElementById('ifScore').textContent     = data.isolation_forest_mean_score.toFixed(4);
                    document.getElementById('ifThreshold').textContent = data.isolation_forest_threshold.toFixed(4);
                    document.getElementById('ifDecision').textContent  = data.if_model_decision;
                    document.getElementById('aeScore').textContent     = data.autoencoder_mean_score.toFixed(4);
                    document.getElementById('aeThreshold').textContent = data.autoencoder_threshold.toFixed(4);
                    document.getElementById('aeDecision').textContent  = data.ae_model_decision;
...
async def predict(file: UploadFile = File(...)):
    ...
    logger.info(f"[/predict] Request received | filename={file.filename} | content_type={file.content_type}")
    ...
    logger.info(f"[/predict] Step 1: File saved to temp | path={tmp_path} | size={tmp_path.stat().st_size} bytes | elapsed={time.time()-t:.2f}s")

    # Step 2: Feature extraction
    t = time.time()
    logger.info("[/predict] Step 2: Starting feature extraction...")
    df = build_feature_df(tmp_path)
    logger.info(f"[/predict] Step 2: Feature extraction done | df.shape={df.shape} | elapsed={time.time()-t:.2f}s")

    # Step 3: Isolation Forest scoring
    t = time.time()
    logger.info("[/predict] Step 3: Running Isolation Forest...")
    if_scores = -IF_BUNDLE["pipeline"].score_samples(df[IF_BUNDLE["feature_columns"]])
    if_mean = float(np.mean(if_scores))
    logger.info(f"[/predict] Step 3: IF done | mean_score={if_mean:.4f} | elapsed={time.time()-t:.2f}s")

    # Step 4: Autoencoder scoring
    t = time.time()
    logger.info("[/predict] Step 4: Running Autoencoder...")
    ...
    logger.info(f"[/predict] Step 4: AE done | mean_score={ae_mean:.4f} | elapsed={time.time()-t:.2f}s")

    # Step 5: Decision logic
    t = time.time()
    logger.info("[/predict] Step 5: Applying decision logic...")
    ...
    logger.info(f"[/predict] Step 5: Decision done | if_result={if_result} | ae_result={ae_result} | final_label={label} | elapsed={time.time()-t:.2f}s")

    total_elapsed = time.time() - request_start
    logger.info(f"[/predict] DONE | predicted_label={label} | total_time={total_elapsed:.2f}s")
